...
using Jonesie.Web.Module.Pages.Controllers;

namespace Jonesie.Web.Module.Pages
{
  /// <summary>
  ...
  public class Module : IViewModule
  {
    ...
    public Type[] Controllers
    {
      get
      {
        return new Type[] { typeof(PageController) };
      }
    }
    ...
    public void InternalInit()
    {
      if (!_initDoneOnce)
      {
        ...
        DatabaseManager.UpdateSchema("Pages", 1, () => { DatabaseManager.ExecuteDDL(Properties.Resources.Schema_Pages_1); });

        // make sure we have the default menu set for this module
        if (!NavigationRepository.GetOptions().Items.Any(no => no.Controller == "Page" && no.Action == "_Admin"))
        {
